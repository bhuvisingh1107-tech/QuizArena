...
class LiveRoomRepository:
    ...
    def get_by_id(self, room_id: UUID) -> LiveRoom | None:
        stmt = (
            select(LiveRoom)
            .options(
    ...
    def has_hosting_room(self) -> bool:
        """True if any room is in an RS-003 hosting state."""
        hosting = [
            RoomState.SETUP,
            RoomState.LOBBY,
            RoomState.ACTIVE,
    ...
    def get_quiz_for_snapshot(self, quiz_id: UUID) -> Quiz | None:
        stmt = (
            select(Quiz)
            .options(
